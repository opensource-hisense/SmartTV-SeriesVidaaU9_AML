...
public:
  ...
  // move from private func -> public func Ackerman
  int openConnection(); // result values: -1: failure; 0: pending; 1: success
  int registerWfdCallback(responseHandler* handler);
...
protected:
  RTSPClient(UsageEnvironment& env, char const* rtspURL,
	     int verbosityLevel, char const* applicationName, portNumBits tunnelOverHTTPPortNum, int socketNumToServer, Boolean wfd_flag = false);
...
protected:
  ...
  responseHandler*  mWfdResponseHandler;
  Boolean fwfd_flag;
private:
