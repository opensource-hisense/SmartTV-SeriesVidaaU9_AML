...
RTSPClient::RTSPClient(UsageEnvironment& env, char const* rtspURL,
		       ...
		       portNumBits tunnelOverHTTPPortNum, int socketNumToServer, Boolean wfd_flag)
  : Medium(env),
    ...
    fTLS(*this),mWfdResponseHandler(NULL),fwfd_flag(wfd_flag) {
  setBaseURL(rtspURL);
...
unsigned RTSPClient::sendRequest(RequestRecord* request) {
  ...
  do {
    ...
    char const*  cmdFmt;
    unsigned cmdSize;
    if (fwfd_flag && strcmp(request->commandName(), "OPTIONS") == 0) {
        cmdFmt =
          "%s %s %s\r\n"
          "CSeq: %d\r\n"
          "%s"
        ...
        cmdSize = strlen(cmdFmt)
            + strlen(request->commandName()) + strlen("*") + strlen(protocolStr)
            + 20 /* max int len */
    ...
    if (fwfd_flag && strcmp(request->commandName(), "OPTIONS") == 0) {
    sprintf(cmd, cmdFmt,
            request->commandName(), "*", protocolStr,
            request->cseq(),
            authenticatorStr,
            fUserAgentHeaderStr,
            extraHeaders,
...
void RTSPClient::handleRequestError(RequestRecord* request) {
  ...
  if (fwfd_flag) {
    char* resultString = strDup(envir().getResultMsg());
    if (request->handler() != NULL) (*request->handler())(this, resultCode, resultString);
    delete[] resultString;
  } else {
...
void RTSPClient::handleResponseBytes(int newBytesRead) {
  ...
  do {
    ...
    do {
      ...
      char* wfdRequest = NULL;
	  if (fwfd_flag) {
        wfdRequest = strDup(headerDataCopy);
      }
      char* lineStart;
	...
	nextLineStart = getLine(lineStart);
      ...
      if (!parseResponseCode(lineStart, responseCode, responseStr)) {
        if (fwfd_flag) {
            char const* CSeqParamsStr;
            unsigned cseqCode = 0;
            do {
                lineStart = nextLineStart;
                nextLineStart = getLine(lineStart);
                if (checkForHeader(lineStart, "CSeq:", 5, CSeqParamsStr)) {
                    if (sscanf(CSeqParamsStr, "%u", &cseqCode) == 1) {
                        if (mWfdResponseHandler != NULL) {
                            envir() <<  "WfdResponseHandler => cseqCode: \"" << cseqCode << "\" \n";
                            envir() <<  "WfdResponseHandler => wfdRequest: \"" << wfdRequest << "\" \n";
                            (*mWfdResponseHandler)(this, cseqCode, wfdRequest);
                        }
                        break;
                    } else {
                        envir() << " lineStart: \"" << lineStart << "\" \n";
                    }
                }
            } while (lineStart[0] != '\0' && nextLineStart[0] != '\0' && nextLineStart != NULL);
            if (wfdRequest != NULL) {
                delete[] wfdRequest;
            }
        } else {
      ...
      envir() <<  " responseCode: \"" << responseCode << "\" \n";
