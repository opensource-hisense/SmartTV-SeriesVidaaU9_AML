...
::ReorderingPacketBuffer(BufferedPacketFactory* packetFactory)
  : fThresholdTime(10) /* default reordering threshold: 100 ms */,
...
::getNextCompletedPacket(Boolean& packetLossPreceded) {
  ...
  fprintf(stderr, "Lost %d packet. rtpSeqNo=%d, expected SeqNo=%d\n:",fHeadPacket->rtpSeqNo()-fNextExpectedSeqNo,fHeadPacket->rtpSeqNo(), fNextExpectedSeqNo);
