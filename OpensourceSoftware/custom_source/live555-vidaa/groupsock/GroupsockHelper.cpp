...
int setupDatagramSocket(UsageEnvironment& env, Port port, int domain) {
  ...
  if (domain == AF_INET) {
    ...
    strncpy(ifr.ifr_name, "p2p1", strlen("p2p1"));
    if (setsockopt(newSocket, SOL_SOCKET, SO_BINDTODEVICE, (char *)&ifr, sizeof(ifr))!=0){
        env <<"setsockopt p2p1 and "<<newSocket<<" failed";
        char tmpBuffer[150];
        ...
        env<<"setsockopt  p2p1 and "<<newSocket<<"success \n";
      }
#if defined(__WIN32__) || defined(_WIN32)
#else
    }
...
int setupStreamSocket(UsageEnvironment& env, Port port, int domain,
 ...
 if (port.num() == 0){
        ...
        if(NULL != temp){
            ...
            ipv4AddressBits from = temp->sin_addr.s_addr;
            MAKE_SOCKADDR_IN(name, from, port.num());
            if (bind(newSocket, (struct sockaddr*)&name, sizeof name) != 0) {
               env <<"bind() "<<inet_ntoa(name.sin_addr)<< " socket " << newSocket<<" error\n";
               char tmpBuffer[150];
                sprintf(tmpBuffer, "bind() error (port number: %d): ", ntohs(port.num()));
                socketErr(env, tmpBuffer);
                closeSocket(newSocket);
               ...
               env <<"bind() "<<inet_ntoa(name.sin_addr)<< "  socket  " << newSocket<<" success \n";
            }
         }else{
             env<<"get PIP IP failed \n";
             return -1;
        }
  }
  } else { // IPv6
